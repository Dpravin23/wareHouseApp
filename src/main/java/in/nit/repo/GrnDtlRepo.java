...
public interface GrnDtlRepo extends JpaRepository<GrnDtl, Integer> {
    ...
    @Query("SELECT status from GrnDtl")
    public List<String> allGrnStatus();
    @Query("SELECT count(status) from GrnDtl where status=:status")
    public Integer countGrnStatus(String status);
}
